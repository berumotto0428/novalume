...
| `file_parsers.py` | 多格式解析 | pymupdf/pdf/python-docx/openpyxl/python-pptx 提取文本 + RecursiveCharacterTextSplitter 分块（1000字/块，200重叠） |
...
process_document():
  ...
  2. RecursiveCharacterTextSplitter 分块 (1000字, 200重叠)
  3. 每块匹配页码 (前80字指纹)
...
用户提问
    ↓
POST /chat (SSE):
  1. vector_service.query() → ChromaDB 查 FETCH_COUNT=30 → BM25关键词评分 → 稠密+稀疏排名 → RRF融合排序 → 低于最佳 chunk×min_score_ratio 的过滤 → 取前 MAX_CHUNKS 个 → 去重
  2. 若结果为空 → "未找到相关内容"
...
Dense (cosine) + Sparse (jieba关键词) → RRF融合
                ↓
分数 = (1 - keyword_weight)/(RRF_K + dense_rank) + keyword_weight/(RRF_K + sparse_rank)
```
